...
3. 等待草稿保存状态稳定，刷新草稿编辑页，再次验证字数、正文重复次数和正文图片数。
4. 正文图不走含图 Markdown/DOCX 导入，改用浏览器逐张定位插入。
5. 插图数量和位置按文章表达核心决定；每张图都重新定位光标，不复用上一张图的光标状态。
6. 每次只上传当前单张图，等待“已上传 1 张图片”，再点击“插入图片”。
7. 每插入一张图后立即验证图片数、`添加图片注释` 占位数、正文重复次数、字数和图片前后段落。
8. 若出现追加正文、字数为 0、图片堆叠或弹窗未关闭，立即停止并恢复干净正文，不继续叠加操作。
