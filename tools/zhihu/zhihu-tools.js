...
function buildBrowserPlaybook({ title }) {
  return [
    ...
    '6. 等待草稿保存状态稳定，确认页面显示“刚刚 · 草稿”或类似草稿状态。',
    '7. 刷新草稿编辑页，重新验证标题、字数、正文重复次数和正文图片数，确保导入结果被知乎内部状态接受。',
    '8. 如果发现正文重复或字数异常，立即停止，不要继续插图；先恢复到干净正文。',
    '',
    '## 正文插图稳定流程',
    '',
    '- 图位按文章表达核心决定，不固定 3 张；每张图都要有明确目标段落。',
    '- 每张图开始前重新定位光标到目标段落后或目标小标题前，不复用上一张图的光标状态。',
    '- 点图片按钮后上传当前单张图片，等待弹窗出现“已上传 1 张图片”。',
    '- 看到“已上传 1 张图片”后必须点击“插入图片”；只上传不点击插入不会进入正文。',
    '- 每插入一张图后立即验证：插入后等待数秒，检查图片数递增 1、`添加图片注释` 占位数递增 1、正文仍只出现 1 份、字数不变、图片前后段落符合预期。',
